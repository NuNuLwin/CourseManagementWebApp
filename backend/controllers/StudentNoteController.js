...
const addNote = asyncHandler(async (req, res) => {
  ...
  const studentnote = await studentNote
    .where("user")
    .equals(user)
    .where("course")
    .equals(course)
  ...
  if (studentnote.length !== 0) {
    ...
    const updateNote = await studentNote.updateOne(
      { _id: studentnote[0]._id },
      { notes: notelist }
    );

    //return note as response
    if (updateNote) {
      const notelist = await studentNote
        .where("user")
        .equals(user)
        .where("course")
        .equals(course)
...
const shareNote = asyncHandler(async (req, res) => {
  ...
  const addShareMember = await studentNote.updateOne(
    { _id: note },
    { members: memberlist }
  );
...
const getSharedNotes = asyncHandler(async (req, res) => {
  ...
  for (let i = 0; i < allNotes.length; i++) {
    // console.log(
    //   "getSharedNotes allNotes[i].file " + allNotes[i].file + " file " + file
    // );
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    for (let j = 0; j < notes.length; j++) {
      ...
      if (files[i].file.equals(notes[j].file)) {
        // console.log(
        //   "files[i] and notes[j] equal " + files[i].file + " " + notes[j].file
        // );
        // loop in notes
